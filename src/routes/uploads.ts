...
uploadRoutes.post("/api/uploads", requireRole("write"), async (c) => {
  ...
  const initialExisting = await c.env.CACHE_BUCKET.head(input.key);
  emitMetric("r2_get", { key: input.key, kind: "nar", operation: "head", status: initialExisting ? 200 : 404, bytes: 0, directUpload: true });
  if (initialExisting) {
  ...
  const indexed = await getObject(c.env, input.key);
  if (indexed?.state === "deleting") throw new AppError("object_deleting", "The object is currently being deleted", 409);
  const stale = await expireStaleUploadSession(c.env, input.key);
