...
import { copyR2Object, createPresignedPut, directUploadTtl } from "./presign";

export const DIRECT_UPLOAD_STAGING_PREFIX = "_nix_uploads/";
export const MAX_DIRECT_UPLOAD_BYTES = 5 * 1024 * 1024 * 1024;
const R2_COPY_PROMOTION_THRESHOLD_BYTES = 100 * 1024 * 1024;
...
async function compareOrRepairExisting(
  ...
  const completedSession = await finalizeDirectUpload(env, session, result.object, result.sha256, false);
...
async function finalizeDirectUpload(
  ...
  object: R2Object,
  sha256: string,
  cleanupObjectOnFailure: boolean,
  ...
  const transaction = await env.DB.batch([
    env.DB.prepare(
      `INSERT INTO objects (r2_key, kind, etag, sha256, size, uploaded_at, state, narinfo_ref_count, version_member_count)
       SELECT ?, 'nar', ?, ?, ?, ?, 'ready', 0, 0
       WHERE EXISTS (SELECT 1 FROM upload_sessions WHERE id = ? AND status = 'issued')
    ...
    ).bind(session.r2_key, object.httpEtag, sha256, object.size, timestamp, session.id),
    env.DB.prepare(
      `UPDATE upload_sessions
       ...
       WHERE id = ? AND status = 'issued'
         AND EXISTS (
           ...
           WHERE r2_key = ? AND kind = 'nar' AND state = 'ready'
             AND etag = ? AND sha256 = ? AND size = ?
         )`,
    ).bind(object.httpEtag, timestamp, timestamp, session.id, session.r2_key, object.httpEtag, sha256, object.size),
  ]);
  if ((transaction[1]?.meta.changes ?? 0) === 1) {
    return {
      ...session,
      status: "completed",
      object_etag: object.httpEtag,
      error_code: null,
  ...
  const current = await getUploadSession(env, session.id);
  if (current?.status === "completed") return current;
  if (cleanupObjectOnFailure) await env.CACHE_BUCKET.delete(session.r2_key);
  if (current?.status === "revoked") {
    throw new AppError("upload_revoked", "The direct-upload session was revoked because its final key was deleted", 409);
  }
  ...
  throw new AppError("upload_session_failed", "The upload session could not be completed", 503);
}
...
export async function completeUploadSession(env: Bindings, id: string): Promise<DirectUploadResult> {
  ...
  if (session.status === "completed") {
    ...
    const digest = await hashStream(current.body);
  ...
  if (!owner) throw new AppError("upload_in_progress", "Another upload or deletion for this object is in progress", 409);
  try {
    ...
    if (staging.size >= R2_COPY_PROMOTION_THRESHOLD_BYTES) {
      await copyR2Object(env, session.staging_key, session.r2_key);
      object = await env.CACHE_BUCKET.head(session.r2_key);
      emitDirectR2Get(session.r2_key, "head", object);
      if (!object) throw new AppError("upload_race", "The R2 server-side promotion completed without an observable object", 503);
    } else {
      const promoteBody = await env.CACHE_BUCKET.get(session.staging_key);
      emitDirectR2Get(session.staging_key, "get", promoteBody);
      if (!promoteBody?.body) throw new AppError("upload_not_ready", "The direct-upload object could not be read from R2", 503);
      object = await env.CACHE_BUCKET.put(session.r2_key, promoteBody.body, {
    ...
    const completedSession = await finalizeDirectUpload(env, session, object, incoming.sha256, true);
